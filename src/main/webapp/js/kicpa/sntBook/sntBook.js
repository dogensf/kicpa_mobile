...
sntBook.lectureInit = function(){
	...
	$("button.btn-round-sm").on("click",function(){
		...
		if($("#gbn").val() == 'LISTPAGE2'){
			...
			text.gray += '입금대기는 수강신청이 되지 않은 상태입니다.<br/>';
			text.gray += '수강료를 입금하시면 수강신청이 완료됩니다.[입금계좌:국민은행 011-25-0016-100 (예금주 : 한국공인회계	사회), 카드결제 불가]<br/>';
//			text.gray += '[입금계좌:국민은행 011-25-0016-100 (예금주 : 한국공인회계	사회), 카드결제 불가]<br/>';
			text.gray += '※ 다만, 선택하신 수강신청과목에 대하여 총 수강료를한번에 본인명의로 입금해 주셔야 하며, 수강신청 후 2일이내에 입금내역이 확인되지 않는 경우 수강신청은 자동취소됨을 알려드리오니 이 점 양지하여 주시기 바랍니다.<br/>';
