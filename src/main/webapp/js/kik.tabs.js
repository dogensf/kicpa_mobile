	...
	function($){
		...
		$.fn.dynatabs = function(options){
			...
			$.fn.bindTabs = function(){
				$.each(tabs.find("li > a"), function(idx, a){
					...
					if(settings.showCloseBtn && $(a).find("span").length == 0 && idx !=0)
					{
						$.fn.addCloseBtn(a);
					}
				});
			...
			$.fn.addCloseBtn = function(ahref){
				...
				if(ahref != null)
				{
					...
					if(key.length > 0)
					{
						...
						$(ahref).append('<span class="btn-close"><i class="icon close"></i></span>');
			...
			$.fn.addNewTab = function(defaults, tabBody){
				var addTab = 'Y';
				$.fn.debug('tabBody ID : ' + tabBody);
				settings.tabBodyID = tabBody;
				tabs = $("#"+defaults.tabID);
				//탭 열려 있는지 확인 처리 
				$.each(tabs.find("li > a"), function(idx, a){
					if(defaults.tID + '_tab' == $(a).attr("id")){
						//alert($(a).attr("id"));
						//$(a).tab('show');
						$(this).tab('show');
						addTab = 'N';
						//Global_grid_resize(defaults.tID+'.fn_init_resize_AUIGrid'); 
						//findMenu($(a).text());
						return false;
					}					
				});
				...
				if(addTab != 'N'){
					...
					else if(defaults.type === 'ajax')
					{
						if(defaults.url.length > 0 && defaults.method.length > 0 && defaults.dtype.length > 0)
						{
							...
							//create the title tag
							var a = $("<a class=\"nav-link\" data-toggle=\"tab\" role=\"tab\" aria-selected=\"true\" />");
							
							//create data div
							var div = $("<div class=\"tab-pane fade show\" role=\"tabpanel\" />");
							var len = tabs.find("li").length + 1;
							$(a).attr('href', '#tabview' + '_' + defaults.tID);
							$(a).attr('id', defaults.tID + '_tab');
							$(a).attr('aria-controls',defaults.tID + '_tab');
							$(a).text(defaults.tabTitle);
							$(div).attr('id', 'tabview' + '_' + defaults.tID);
							$(div).attr('aria-labelledby', defaults.tID +'_tab');						
							$(li).append(a);
							$(div).html("Loading...");
							//append to tab list
							tabs.append(li);
		...
		$.addDynaTab = function(options){
			...
			if(settings.tabID.length > 0)
			{
				...
				//$.fn.addNewTab(settings, tabList[settings.tabID]);D
				$.fn.addNewTab(settings, 'addajaxtabbody');
