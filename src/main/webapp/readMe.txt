...
1. 목자
	개요
 		...
 		- Directory 표준
2. 개요
	- 프로젝트의  개발 시 개발생산성 향상 및 운영의 효율화를 위해 반드시 준수되어야 하는 준수사항.
		1) 제시된 표준은 철저히 준수한다.
...
3. 명명규칙 표준
	- 시스템을 개발하는데 있어 표준 Naming Rule을 적용하여 개발자 및 운영자가 분석 및 코딩하는데 있어 좀더 쉽게 접근할 수 있도록 한다.

		1) 시스템 개발코드
			시스템 영역별 코드를 분류한다. ex) ~ 시스템 -> KICPA
		...
		- 축약형(Abbreviation) 과 두문자어형(Acronym)을 이름에 사용할 경우에는 전부 대문자로 지정하지 않는다.
		ex)
	...
	4) Package, Java class, Jar
		...
		-> Source 부분 -> package  adminwork.kicpa.업무대분류.중분류.기능구분;	
		-> 중분류 : 업무중분류 혹은 소분류에 적합한 이름을 명명한다.	 
		            예) adminwork.kicpa.exam.web (controller package)
		          adminwork.kicpa.exam.service (model package)	
	...
	5) 프레임워크 적용 시 설정파일 명명규칙
		...
		예) exam.java  -  examInsertDAO
        
	6) 기타 Resource 명명규칙

 		- DATA
 		  - 모든 통신은 .ajax 를 기본으로 사용한다.
 		  - 공통모듈에서 제공하는 ajax모듈 및 개인역량에 따른 개발론을 적용한다.
 		    단, 모든 ajax가 처리 되는 곳에는 function명 postfix에 _ajax를 명시한다.
 		   예) setExamNo_ajax() , getExamList_ajax()
        - 화면 구성
          - 기본 화면은 TILES 프레임워크 사용
          - 화면개발은 업무단 상세화면 개발.
          - modal은 페이지 제일 하단에 위치하며, _modal ,Modal 등 postfix 명시 
          - 기본제공 템플릿 을 분석하여 사용한다.
          - 메뉴 연동은 공통메뉴호출 함수에 servlet정보를 메핑한다.
                 예)addNewTab(url,params) get방식
                  
		- Html
		 /WEB-INF/jsp/kicpa 밑에 자바 CLASS명을 따라 명칭을 부여한다.
		   일반 명칭은  .jsp 로 생성하되, 팝업인경우 _pop.jsp or Pop.jsp 으로 postpix로 명명한다.
		 “고유코드”는 대문자로 하고 “.jsp” 첫 자는 대문자로 하고 대소를 구분하여 의미를 가지는 명칭으로 한다.
		 확장자는 .jsp로 정하기로 한다.(.htm불가)
		예) “Exam” + “Test” + “.jsp”
		“Exam” + “Test” + “.jsp”
		...
		- Javascript
		 Java Script function명은 연관 업무와 기능을 위주로 고유하게 명명한다.
		     반드시 postfix 에 해당 메뉴의 명칭이 들어 가야 한다. 
...
각 메뉴별  자바스크립트들은 관련된 기능에 따라 하나로 묶어 메뉴명과 유사한 고유한 이름을 부여한 파일로 관리한다.

예) DateCommon.js, EventCommon.js, exam.js
 
