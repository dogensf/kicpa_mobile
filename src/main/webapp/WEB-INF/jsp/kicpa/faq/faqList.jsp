			...
			<input name="faq_cate"  type="hidden" value="<c:out value='${searchVO.faq_cate}'/>"/>
		      <div class="faq-menus">
		        <ul>
		          ...
		          <li><a href="javascript:fn_select_List_cate('제증명');" <c:if test="${searchVO.faq_cate == '제증명'}">class="active"</c:if>>제증명</a></li>
		            ...
		            <dd>
		              <span class="compact-select" style="width: 150px;">
		                <select name="searchCnd" id="searchCnd">
		                  ...
		                  <option value="0" <c:if test="${searchVO.searchCnd == '1'}">selected="selected"</c:if>>제목명</option>
		                  <option value="1" <c:if test="${searchVO.searchCnd == '2'}">selected="selected"</c:if>>담당자</option>
		                  <option value="2" <c:if test="${searchVO.searchCnd == '3'}">selected="selected"</c:if>>담당부서</option>	
		                </select>
		              </span>
		            </dd>
		            <dd><span class="compact-input" style="width: 600px;"><input type="text" name="searchWrd" id="searchWrd" placeholder="검색어 입력" value="<c:out value="${searchVO.searchWrd}"/>"  onkeypress="press(event);"></span></dd>
		          </dl>
	              ...
	              <select name="faq_job" id="faq_job">
	              	...
	              	<c:forEach var="code" items="${ccode}" varStatus="v">
	               		<c:if test="${fn:contains(code.level1, searchVO.faq_cate) }">
	               			<option value="${code.level2 }">${code.level2 }</option>		
	               		</c:if>               		
	               	</c:forEach>
        ...
        <table class="tb-bg">
          ...
          <tbody>
	          <c:forEach var="result" items="${resultList}" varStatus="status">
	              ...
	              <td><a href="<c:url value='/kicpa/faq/faqDetail.do?faq_id=${result.faq_id}&pageIndex=${searchVO.pageIndex}'/>"><c:out value="${result.faq_sj}"/></a></td>
